...
**Written:** 2026-08-05
**Last revised:** 2026-08-19 — host egress corrected; the applied rule set is
now written and asserted by the repository
**Status:** **implemented and enforced on `pve1`.** This document opened as a
design with a measured failure to design against, and that framing is now out
of date. The *baseline* was measured 2026-08-05; the *consequence* — lab guests
reaching the management address, each other across sections, and a recursive
resolver — was measured 2026-08-07 and closed the same day by hand. Since
2026-08-18 the rule set is generated by
`ansible/playbooks/controller-bootstrap-firewall.yml` and checked by
`controller-assert-isolation.yml`, both applied to `pve1`, with all seventeen
probe rows passing on 2026-08-19.

Sections below are kept in the order they were learned rather than rewritten
into a clean design, because the measurements are the evidence and several of
them contradict what a reasonable person would have assumed.
...
- **`chrony` is a live, continuous egress path right now.** It is reaching
  ...
  **Superseded 2026-08-19, and the new state is worse than either intent.**
  Enabling the firewall silently closed udp/123, and the host egress rules added
  on 2026-08-18 deliberately did not reopen it. `chrony` is still enabled and
  still trying: `chronyc sources` shows six pool servers at `Reach 0`, and
  `chronyc tracking` reports `Stratum 0`, `Reference ID 00000000` —
...
### Applied to `pve1`, 2026-08-18 — the repository is now the source of truth

`controller-bootstrap-firewall.yml` was run outside check mode for the first
time. All four files — `cluster.fw`, `host.fw`, `950.fw`, `955.fw` — are now
generated and carry the `Managed by ... Do not edit by hand` header. Re-verified
on 2026-08-19: both tiers pass, all seventeen rows, control included.

Everything below this line that speaks of "the hand-placed rules" is describing
the state that preceded this, kept because the diffs against it are what made
the swap safe to perform.

**The apply is also what gave the node its own egress back** (see "Optional
egress" below). The same probe run that confirmed the host can reach GitHub and
Debian returned `blocked` on both internet rows from inside a pod, which is the
result that matters: opening the host's outbound path on the uplink took nothing
away from lab containment.

#### Two findings from the days either side of the apply

**A runtime repair is not a repair.** The cache fix of 2026-08-18 was applied to
the live container in part by hand — the `apt-cacher-ng` drop-in was written,
but the route unit was only restarted, not rewritten. The next reboot lost it
and the cache broke a second time in as many days. Running the playbook is what
made it durable. This is the reproducible-from-recipe argument demonstrated at
the smallest possible scale, and it cost two failures to learn.

**The fail-closed design was observed working.** On that second failure the
`Requires=` drop-in meant `apt-cacher-ng` did not start at all, rather than
starting and binding loopback. The difference is diagnosability: a service that
is plainly down names itself in `systemctl status`, and tier 1 quoted the cause
in its failure message. The original failure mode — `active` while serving
nobody — took a live packet probe to detect.
